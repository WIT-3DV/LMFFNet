...
def main():
    ...
    model_idx = -1
    weights = [1, 1]
    model_folder = 'TransMorph_MSSAM_LKA_ncc_{}_diffusion_{}/'.format(weights[0], weights[1])
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            x_def, flow = model(x_in) # x_def是out，flow是形变场
            x_seg_oh = nn.functional.one_hot(x_seg.long(), num_classes=46)
            x_seg_oh = torch.squeeze(x_seg_oh, 1)
            x_seg_oh = x_seg_oh.permute(0, 4, 1, 2, 3).contiguous()
            #x_segs = models.spatial_trans(x_seg.float(), flow.float())
            x_segs = []
            for i in range(46):
